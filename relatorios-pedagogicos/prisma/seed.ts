...
async function main() {
  ...
  const turmasComAlunos: Record<string, string[]> = {
 '1601': [
  "ADRYELLEM SANTANA DE FRANÇA",
  "ARTHUR BRAGANÇA DOS SANTOS",
...
'1801': [
  ...
  'EMILLY VICTÓRIA BRITO ROSÁRIO',
  'ENDERSON GABRIEL MACHADO RODRIGUES',
  ...
  'PEDRO HENRIQUE LUIZ CAMARGO',
  'RENNAN MOREIRA SANTOS',
...
'1902': [
  'ALEX SOARES RODRIGUES DA SILVA',
  ...
  'NATHALIA DE ANDRADE ALVES',
  'NICOLLE OLIVEIRA TORRES DA SILVA',
  ...
  // 6. Criar aluno de teste e relatórios
  console.info('👩‍💻 Criando alunos de teste e relatórios...');
  const professoresDb = await prisma.professor.findMany();
  for (const turmaNome of turmasNomes) {
    const turma = await prisma.turma.findUnique({ where: { id: turmasMap[turmaNome] }, include: { materias: true } });
    if (!turma) continue;

    const alunoTeste = await prisma.aluno.create({
      data: {
        name: `ALUNO TESTE ${turmaNome}`,
        turmaId: turma.id,
        matricule: `TESTE-${turmaNome}`,
        active: true,
      },
    });
    ...
    for (const materia of turma.materias) {
      await prisma.relatorio.create({
        data: {
          conteudo: `Relatório de teste do ${alunoTeste.name} na matéria ${materia.name}.`,
          status: 'ENVIADO',
          alunoId: alunoTeste.id,
          professorId: professoresDb[0].id,
